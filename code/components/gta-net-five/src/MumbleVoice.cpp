...
static std::shared_ptr<lab::AudioContext> getAudioContext(int playerId)
{
	...
	// WARNING: This is a implementation detail
	if (!g_mumble.connected || serverId == 0)
	{
		return {};
	}
...
static HookFunction hookFunction([]()
{
	...
	rage::scrEngine::OnScriptInit.Connect([]()
	{
		...
		static auto InvalidTargetIdWarning = [] (const std::string_view& nativeName)
		{
		...
		fx::ScriptEngine::RegisterNativeHandler("MUMBLE_ADD_VOICE_CHANNEL_LISTEN", MakeMumbleNative([](fx::ScriptContext& context)
		{
			...
			// TODO: See if we actually want to have a warning here, since this wouldn't cause any noticeable issues, unlike voice targeting which players might
			// get confused on why it doesn't work
			// else
			// {
