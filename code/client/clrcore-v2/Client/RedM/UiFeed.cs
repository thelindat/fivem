...
namespace CitizenFX.RedM
{
	public static class UiFeed
	{
		...
		private static int GenerateFeed4Args(Hash nativeHash, UiFeedInfo feedInfo, UiFeedData feedData)
		{
			int retVal;
			unsafe
			{
				...
				retVal = Natives.Call<int>(nativeHash, &dur, &notification, true, true);
			}
			return retVal;
		}

		[SecuritySafeCritical]
		private static int GenerateFeed3Args(Hash nativeHash, UiFeedInfo feedInfo, UiFeedData feedData)
		{
			int retVal;
			unsafe
			{
				UnsafeUiInfo dur = new UnsafeUiInfo(feedInfo);
				UnsafeUiFeedNotification notification = new UnsafeUiFeedNotification(feedData);
				retVal = Natives.Call<int>(nativeHash, &dur, &notification, true);
			}
			return retVal;
		}
